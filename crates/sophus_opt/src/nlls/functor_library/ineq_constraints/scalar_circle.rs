...
/// Circle containment inequality constraint for a 2D point variable.
///
/// The constraint ensures that a 2D point stays inside a circle:
///
/// `h(p) = radius² − ‖p − center‖² >= 0`
///
/// Positive means the point is inside the circle (feasible).
#[derive(Clone, Debug)]
pub struct ScalarCircleInsideConstraint {
...
impl HasIneqConstraintFn<2, 1, (), VecF64<2>> for ScalarCircleInsideConstraint {
    ...
    fn eval(
        ...
        let h = self.radius * self.radius - diff.dot(&diff);

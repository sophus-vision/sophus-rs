...
pub enum NllsError {
    ...
    /// Phase-1 feasibility search failed to find a feasible point.
    #[snafu(display(
        "Phase-1 failed: could not find a feasible starting point (min_h = {min_h:.4e}). \
         The initial point may be at a degenerate location (e.g. center of an ellipse \
         where ∇h = 0). Try a different starting point."
    ))]
    Phase1Failed {
        /// Minimum constraint value at termination (negative = infeasible).
        min_h: f64,
    },
}

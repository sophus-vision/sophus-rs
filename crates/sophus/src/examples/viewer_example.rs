...
use sophus_viewer::renderables::frame::Frame;
...
pub fn make_distorted_frame() -> Frame {
    ...
    let pinhole_cam = DynCameraF64::new_pinhole(
        &VecF64::from_array([focal_length, focal_length, cx, cy]),
        image_size,
    );
    ...
    for v in 0..image_size.height {
        for u in 0..image_size.width {
            ...
            if p_on_z1[0].abs() < 0.5 {
                *img.mut_pixel(u, v) = nalgebra::SVector::<u8, 4>::new(255, 0, 0, 255);

                if p_on_z1[1].abs() < 0.3 {
                    *img.mut_pixel(u, v) = nalgebra::SVector::<u8, 4>::new(0, 0, 255, 255);
                }
            }
        }
    }

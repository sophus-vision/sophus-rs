...
/// Threshold for pivot condition ratio below which BK fallback activates
/// on indefinite blocks. A ratio of 1e-10 means we've lost 10 digits of
/// precision — BK pivoting can recover stability.
...
impl BlockDiagonalLdltSystem {
    ...
    pub fn right_solve_inplace(&self, x: DMatrixViewMut<'_, f64>, idx: PartitionBlockIndex) {
        ...
        if let Some(ref bk) = self.bk_factors[flat] {
            ...
            // Copy back to mutable view — need to drop x first.
            // Actually x is already a mutable view, copy_from works.
            let mut x = x;
            x.copy_from(&x_owned);
        } else {
            // Standard LDLᵀ path.

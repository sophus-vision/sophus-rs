...
fn symbolic_factorize_l(
    ...
    sub: &BlockMatrixSubdivision,
) -> (Vec<Vec<usize>>, Grid<usize>) {
    let nb = a_lower.block_count();
    let mut etree = EliminationTree::new(a_lower.build_pattern_upper());
    ...
    for col_j in 0..nb {
        ...
        for entry in a_lower.col(col_j).iter() {
    ...
    let np = sub.partitions().specs().len();
    let mut l_region_nf = Grid::new([np, np], 0usize);
    for col_j in 0..nb {
        let col_idx = sub.idx(col_j);
        let col_dim = sub.block_dim(col_idx.partition);
        for &row_i in &l_col_rows[col_j] {
            let row_idx = sub.idx(row_i);
            let row_dim = sub.block_dim(row_idx.partition);
            *l_region_nf.get_mut(&[row_idx.partition, col_idx.partition]) += row_dim * col_dim;
        }
    }
...
impl BlockSparseLdlt {
    ...
    pub fn factorize_impl(
        ...
        let sub = a_lower.subdivision();
        let (l_col_rows, l_region_nf) = symbolic_factorize_l(&a_lower.lower, sub);

        let nb = a_lower.lower.block_count();
        let mut etree = BlockLdltWorkspace::calc_etree(&a_lower.lower);
        let mut ws = BlockLdltWorkspace::new(nb, sub);
        let mut mat_l = BlockSparseLFactorBuilder::with_symbolic(sub, &l_col_rows, &l_region_nf);
        let mut diag_d = BlockDiagonalLdltSystem::zero(sub.partitions().specs(), self.tol_rel);
...
impl BlockSparseLFactorBuilder {
    ...
    pub fn with_symbolic(
        ...
        for row_part in 0..np {
            for col_part in 0..np {

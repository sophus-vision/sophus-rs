...
/// Near-zero threshold for pivot detection in Bunch-Kaufman factorization and solve.
const BK_NEAR_ZERO: f64 = 1e-30;
...
fn eliminate_1x1(ld: &mut DMatrix<f64>, k: usize, n: usize) -> Result<(), BunchKaufmanError> {
    let d_kk = ld[(k, k)];
    if d_kk.abs() < BK_NEAR_ZERO {
...
mod tests {
    ...
    /// Helper: solve A x = b via BK, return x.
    fn bk_solve(a: &DMatrix<f64>, b: &DVector<f64>) -> DVector<f64> {
        let factor = factorize(a).expect("BK factorize failed");
        let mut x = b.as_slice().to_vec();
        factor.solve_slice_inplace(&mut x);
        DVector::from_vec(x)
    }

    #[test]
    fn positive_definite_3x3() {
        // SPD matrix — should use only 1×1 pivots.
        let a = DMatrix::from_row_slice(3, 3, &[4.0, 2.0, 1.0, 2.0, 5.0, 3.0, 1.0, 3.0, 6.0]);
        let b = DVector::from_vec(vec![1.0, 2.0, 3.0]);

        let x = bk_solve(&a, &b);
        let residual = (&a * &x - &b).norm();
        assert!(residual < 1e-12, "PD residual = {residual:.2e}");
    }

    #[test]
    fn indefinite_kkt_4x4() {
        // KKT saddle-point: [H Gᵀ; G -εI].
        // H = [4 1; 1 3], G = [1 1], ε = 1e-8
        let eps = 1e-8;
        #[rustfmt::skip]
        ...
        assert!(
            residual < 1e-10,
            "KKT residual = {residual:.2e} (expected < 1e-10)"
        );
    }

    #[test]
    fn indefinite_needs_2x2_pivot() {
        // Matrix where a 2×2 pivot is needed: zero diagonal with large off-diagonal.
        #[rustfmt::skip]
        ...
        assert!(
            residual < 1e-12,
            "2×2 pivot residual = {residual:.2e} (expected < 1e-12)"
        );
    }

    #[test]
    fn roundtrip_larger_indefinite() {
        // 6×6 indefinite matrix resembling a KKT system with 4 vars + 2 constraints.
        let n = 6;
        ...
        for i in 0..4 {
            a[(i, i)] = 5.0 + i as f64;
            for j in 0..i {
                let v = 0.5 / (1.0 + (i - j) as f64);
                a[(i, j)] = v;
                a[(j, i)] = v;
            }
        }
        // G block (2×4) and -εI (2×2).
        a[(4, 0)] = 1.0;
        a[(0, 4)] = 1.0;
        a[(4, 1)] = 0.5;
        a[(1, 4)] = 0.5;
        ...
        assert!(
            residual < 1e-6,
            "6×6 KKT residual = {residual:.2e} (expected < 1e-6)"
        );
    }
}

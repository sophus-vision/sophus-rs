...
@group(1) @binding(4) var distorted_depth_texture : texture_storage_2d<r32float, write>;
 ...
 fn distort_pixel(view_port_coords_distorted: vec2<u32>, image_size: vec2<f32>, view_port_size: vec2<u32>, background_color: vec4<f32>) {
        let uv_distorted =  vec2<f32>(view_port_coords_distorted) *  vec2<f32>(image_size) / vec2<f32>(view_port_size);
        let uv_undistorted = distorted_to_undistorted(vec2<f32>(uv_distorted), frustum_uniforms);
...
fn distort(@builtin(global_invocation_id) global_id : vec3<u32>) {
    ...
    let image_size = vec2<f32>(frustum_uniforms.camera_image_width, frustum_uniforms.camera_image_height);

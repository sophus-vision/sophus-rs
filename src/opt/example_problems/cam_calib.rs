mod tests {
    ...
    use crate::opt::example_problems::cost_fn::reprojection::ReprojTermSignature;
    use crate::opt::example_problems::cost_fn::reprojection::ReprojectionCostFn;
    use crate::opt::nlls::optimize;
    use crate::opt::nlls::OptParams;
    use crate::opt::robust_kernel::HuberKernel;
    ...
    struct CamCalibProblem {
        ...
        points_in_world: Vec<VecF64<3>>,
        observations: Vec<ReprojTermSignature>,

        true_intrinsics: PinholeCamera<f64>,
        true_world_from_cameras: Vec<Isometry3<f64>>,
        true_points_in_world: Vec<VecF64<3>>,
    }
    ...
    impl CamCalibProblem {
        pub fn new(spurious_matches: bool) -> Self {
            ...
            for i in 0..40 {
                ...
                true_points_in_world
                    .push(true_world_from_cameras[0].transform(&true_point_in_cam0));
        ...
        pub fn optimize_with_two_poses_fixed(&self, intrinsics_var_kind: VarKind) {
            ...
            let up_var_pool = optimize(
                ...
                vec![
                    ...
                    CostFn::new_robust(
                        reproj_obs.clone(),
                        ReprojectionCostFn {},
                        opt::robust_kernel::RobustKernel::Huber(HuberKernel::new(1.0)),
                    ),
                ],
                OptParams {
            ...
            let refined_world_from_robot =
                up_var_pool.get_members::<Isometry3<f64>>("poses".into());
